...
/*
    ...
    start=clock();
    RandomForest forest;
    forest.readTrainSample("wdbc.data");
    forest.readTestSample("test_wdbc.data");
    forest.buildTree(50);
    end=clock();
    ...
    forest.writeForest("tree.txt");
    forest.writeForestModel("tree_model.txt");
    end=clock();
...
*/
    ...
    readForest("/home/qhh/CLionProjects/code/RF/Touch/tree/tree_model_60_50.txt",all_decision_nodes,all_leaf_nodes,all_leaf_values);
    Paillier auth;
    auth.KeyGen();
    SP sp=SP(auth);
    int node_m=0;
    vector<map<int, pair<int, NTL::ZZ >>>enc_all_decision_nodes;
    for(auto i=0;i<all_decision_nodes.size();++i)
    {   map<int,pair<int,NTL::ZZ> >dec_nodes;
        for(auto tmp:all_decision_nodes[i])
        {
            int index=tmp.first;
            int attr=tmp.second.first;
            int64_t value=tmp.second.second;
            NTL::ZZ enc_value=sp.encrypt(NTL::ZZ(value));
            node_m++;
            dec_nodes[index]=make_pair(attr,enc_value);
            cout<<value<<ends<<enc_value<<endl;
        }
        enc_all_decision_nodes.push_back(dec_nodes);
        dec_nodes.clear();
    }
    cout<<"node m "<<node_m<<endl;
    vector<pair<int,vector<int64_t >>>_features;
    vector<pair<int,vector<int64_t >>>features;
    readSample("/home/qhh/CLionProjects/code/RF/Touch/data/data_80_test.csv",_features);
    ...
    for(const auto &enc_feature:enc_features)
    {
        ...
        for(int tree_counts=0;tree_counts<forest_size;tree_counts++)
        {
            ...
            map<int, pair<int, NTL::ZZ >>enc_decision_nodes=enc_all_decision_nodes[tree_counts];
            ...
            sp.setEncDecisionNodes(enc_decision_nodes);
            ...
            gen_circuits_2(ALICE,sp.m,gen,sp);
